#!/usr/bin/env python3
"""Monitor PTT PttEarnMoney and send new-post notifications to Discord."""
...
STATE_FILE = Path(os.getenv("STATE_FILE", "data/seen_posts.json"))
NOTIFY_ON_FIRST_RUN = os.getenv("NOTIFY_ON_FIRST_RUN", "false").lower() == "true"
MAX_SEEN_POSTS = int(os.getenv("MAX_SEEN_POSTS", "1000"))
REQUEST_TIMEOUT = int(os.getenv("REQUEST_TIMEOUT", "20"))

HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (compatible; PttEarnMoneyMonitor/1.0; "
        "+https://github.com/)"
    ),
}
...
def load_state() -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"Cannot read state file {STATE_FILE}: {exc}") from exc
...
def save_state(post_ids: list[str]) -> None:
    ...
    payload = {
        "initialized": True,
        # Preserve order while removing duplicates; retain a bounded history.
...
def discord_payload(post: Post) -> dict[str, Any]:
    description_parts = []
    if post.author:
        description_parts.append(f"作者：`{post.author}`")
    if post.date:
        description_parts.append(f"日期：`{post.date}`")
    if post.push:
        description_parts.append(f"推文：`{post.push}`")
        ...
        "embeds": [
            {
                ...
                "url": post.url,
                "description": "　｜　".join(description_parts) or "發現新文章",
                "color": 0x2F3136,
...
def send_discord(post: Post) -> None:
    ...
    # wait=true makes Discord return an error response if delivery fails.
    separator = "&" if "?" in DISCORD_WEBHOOK_URL else "?"
    webhook_url = f"{DISCORD_WEBHOOK_URL}{separator}wait=true"

    response = requests.post(
        webhook_url,
        json=discord_payload(post),
        timeout=REQUEST_TIMEOUT,
    )

    if response.status_code == 429:
        retry_after = float(response.json().get("retry_after", 1))
        logging.warning("Discord rate limited; retrying in %.2f seconds.", retry_after)
...
def main() -> int:
    ...
    if not state["initialized"] and not NOTIFY_ON_FIRST_RUN:
        ...
        logging.info(
            "First run: stored %d current posts without sending notifications.",
            len(current_ids),
        )
        return 0
    ...
    # PTT index is generally oldest-to-newest, so notify in page order.
    ...
    for post in new_posts:
        ...
        send_discord(post)
        successfully_notified.append(post.post_id)

    # Only mark successfully delivered posts as seen.
    merged_ids = state["seen_ids"] + successfully_notified
    # Also include current IDs during initialization with notification enabled.
    if not state["initialized"]:
        merged_ids = current_ids
    save_state(merged_ids)
